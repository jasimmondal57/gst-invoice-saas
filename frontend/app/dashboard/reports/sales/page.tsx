...
export default function SalesReportsPage() {
  ...
  useEffect(() => {
    ...
    if (!authToken) {
      ...
      fetchSalesReports(authToken, orgId || '', start.toISOString().split('T')[0], end.toISOString().split('T')[0]);
    }
  }, [router]);

  const fetchSalesReports = async (authToken: string, orgId: string, start: string, end: string) => {
    setLoading(true);
    try {
      const response = await fetch(
        `http://localhost:5000/api/v1/invoices?organizationId=${orgId}&startDate=${start}&endDate=${end}`,
  ...
  const handleDateFilter = () => {
    if (startDate && endDate) {
      fetchSalesReports(token, organizationId, startDate, endDate);
    }
  };
  ...
  const stats = {
    totalInvoices: filteredReports.length,
                ...
                <tbody>
                  {filteredReports.map((report, idx) => (
                    <tr key={idx} className="border-b hover:bg-gray-50">
                      <td className="px-6 py-4 text-sm font-medium text-gray-900">{report.invoiceNumber}</td>
                      <td className="px-6 py-4 text-sm text-gray-600">{report.customerName}</td>
                      <td className="px-6 py-4 text-sm text-gray-600">
                        {new Date(report.invoiceDate).toLocaleDateString('en-IN')}
                      </td>
